...
% Copyright 2016 The MathWorks, Inc.
    ...
    if ~isnumeric(A) && ~islogical(A) && ~isa(A,'tabular')
        error(message(string('MATLAB:smoothdata:badArray')));
    end
    if isinteger(A) && ~isreal(A)
        error(message(string('MATLAB:smoothdata:complexIntegers')));
    end
...
% Smooth a single numeric array
    ...
    if ~isnumeric(A) && ~islogical(A)
        error(message(string('MATLAB:smoothdata:nonNumericTableVar')));
    end

    % Dispatch to the correct method
    if method == string('gaussian')
        y = smoothgauss(A,winsz,dim,nanflag,t);
    % Moving mean or median
    elseif startsWith(method, string('mov'))
        spargs = {};
        if ~isempty(t)
            spargs = { 'SamplePoints', t };
        end
        if contains(method, string('mean'))
    ...
    elseif contains(method, string('ess'))
        degree = 1 + ~contains(method, string('wess'));
...
% Parse and check input arguments
    ...
    currentErrorTerm = string('Method');
    ...
    if (argIdx < nargin) && ~charHelper(varargin{1})
        if isa(A, 'tabular')
            error(message(string('MATLAB:smoothdata:noDimForTable')));
        end
        dim = varargin{argIdx};
        if ~(isnumeric(dim) || islogical(dim)) || ~isscalar(dim) || ...
             ~isreal(dim) || ((dim < 1) || (dim ~= round(dim)))
            error(message(string('MATLAB:getdimarg:dimensionMustBePositiveInteger')));
        end
        argIdx = 2;
    end

    methodNames = [string('movmean'), string('movmedian'), string('gaussian'), ...
    string('lowess'), string('loess'),string('rlowess'), string('rloess'), string('sgolay')];
    ...
    if argIdx < nargin
        if charHelper(varargin{argIdx})
            ...
            if nnz(methodID) > 1
                error(message(string('MATLAB:smoothdata:invalidMethod')));
            elseif nnz(methodID) == 1
                method = methodNames(methodID);
                userGaveMethod = true;
                argIdx = argIdx + 1;
                currentErrorTerm = string('Winsize');
                % Window size parsing
                if (argIdx < nargin) && ( isnumeric(varargin{argIdx}) || ...
                    ...
                    argIdx = argIdx + 1;
                    currentErrorTerm = string('Nanflag');
                end
            end
        else
            error(message(string('MATLAB:smoothdata:invalidMethod')));
    ...
    NVPairID = string('NVPair');
    if isa(A, 'tabular')
        NVPairID = NVPairID + string('Table');
    end
    if method == 'sgolay'
        NVPairID = NVPairID + string('SGolay');
    end

    % 'omitnan' / 'includenan' parsing
    userGaveNaNCondition = false;
    if (argIdx < nargin) && charHelper(varargin{argIdx})
        nanflagID = partialMatch([string('omitnan'), string('includenan')], varargin{argIdx});
        if any(nanflagID)
            currentErrorTerm = NVPairID;
            if nanflagID(2)
                nanflag = string('includenan');
            end
    ...
    validParams = [string('SamplePoints'), string('SmoothingFactor')];
    allParams = [validParams, string('DataVariables'), string('Degree')];
    if isa(A, 'tabular')
        validParams(end+1) = string('DataVariables');
    end
    if method == 'sgolay'
        validParams(end+1) = string('Degree');
    end
    ...
    while argIdx < nargin
        if ~charHelper(varargin{argIdx})
            error(message(string('MATLAB:smoothdata:invalid') + currentErrorTerm));
        else
            if currentErrorTerm == string('Winsize')
                currentErrorTerm = string('Nanflag');
            end
        end
        if ~userGaveMethod && any(partialMatch(methodNames, varargin{argIdx}))
            error(message(string('MATLAB:smoothdata:methodAfterOptions')));
        elseif any(partialMatch([string('omitnan', 'includenan')], varargin{argIdx}))
            if userGaveNaNCondition
                error(message(string('MATLAB:smoothdata:duplicateNanflag')));
            else
                error(message(string('MATLAB:smoothdata:nanflagAfterOptions')));
            end
        ...
        elseif ~(nnz(partialMatch(validParams, varargin{argIdx})) == 1)
            if partialMatch(string('DataVariables'), varargin{argIdx}, 2) && ~isa(A, 'tabular')
                error(message(string('MATLAB:smoothdata:DataVariablesArray')));
            elseif partialMatch(string('Degree'), varargin{argIdx}, 2) && (method ~= string('sgolay'))
                error(message(string('MATLAB:smoothdata:degreeNoSgolay')));
            else
                % Error message might be about invalid method, option, or
                % Name-Value pair depending on the input arguments
                error(message(string('MATLAB:smoothdata:invalid') + currentErrorTerm));
            end
        elseif partialMatch(string('SamplePoints'), varargin{argIdx}, 2)
            currentErrorTerm = NVPairID;
            argIdx = argIdx + 1;
            if argIdx >= nargin
                error(message(string('MATLAB:smoothdata:nameNoValue'), 'SamplePoints'));
            else
                if isa(A, 'timetable')
                    error(message(string('MATLAB:smoothdata:SamplePointsTimeTable')));
                end
                t = varargin{argIdx};
                checkSamplePoints(t, A, dim);
            end
        elseif partialMatch(string('SmoothingFactor'), varargin{argIdx}, 2)
            currentErrorTerm = NVPairID;
            if ~isempty(winsz)
                error(message(string('MATLAB:smoothdata:tuneAndWindow')));
            end
            argIdx = argIdx + 1;
            if argIdx >= nargin
                error(message(string('MATLAB:smoothdata:nameNoValue'), 'SmoothingFactor'));
            else
                smoothingFactor = checkSmoothingFactor(varargin{argIdx});
            end
        elseif partialMatch(string('DataVariables'), varargin{argIdx})
            currentErrorTerm = NVPairID;
            argIdx = argIdx + 1;
            if argIdx >= nargin
                error(message(string('MATLAB:smoothdata:nameNoValue'), 'DataVariables'));
            else
            ...
            currentErrorTerm = NVPairID;
            if argIdx >= nargin
                error(message(string('MATLAB:smoothdata:nameNoValue'), 'Degree'));
            else
    ...
    if tIsTimeBased
        ...
        if ~isempty(winsz) && ~isduration(winsz)
            error(message(string('MATLAB:smoothdata:winsizeNotDuration'), class(t)));
        end
    else
        % Non timestamped data
        if ~isempty(winsz) && isduration(winsz)
            error(message(string('MATLAB:smoothdata:winsizeIsDuration')));
        end
    end
...
% Check the window size input parameter
    if ~(isvector(winsz) || isempty(winsz)) || ...
       ~(isnumeric(winsz) || isduration(winsz) || islogical(winsz)) || ...
        (numel(winsz) > 2)
        error(message(string('MATLAB:smoothdata:invalidWinsize')));
    end
    ...
    if isfloat(winsz)
        if ~isreal(winsz)
            error(message(string('MATLAB:smoothdata:noComplexWindows')));
        end
        if issparse(winsz)
            winsz = full(winsz);
        end
    ...
    if (isfloat(winsz) || isduration(winsz)) && any(~isfinite(winsz))
        error(message(string('MATLAB:smoothdata:needsFiniteWindows')));
    end
    if ~isduration(winsz)
        winsz = double(winsz);
    end
...
% Check the sample points input parameter
    ...
    if ~(isvector(t) || isempty(t)) || ...
       ~(isnumeric(t) || isduration(t) || isdatetime(t))
        error(message(string('MATLAB:smoothdata:invalidSamplePoints')));
    end
    ...
    if isempty(t)
        if isempty(A)
            ...
            error(message(string('MATLAB:smoothdata:SamplePointsLength')));
        end
    end
    ...
    if (isfloat(t) || isduration(t)) && any(~isfinite(t))
        error(message(string('MATLAB:smoothdata:needsFinites'),term));
    end
    ...
    if isfloat(t)
        if ~isreal(t)
            error(message(string('MATLAB:smoothdata:noComplex'), '''SamplePoints'''));
        end
        if issparse(t)
            error(message(string('MATLAB:smoothdata:noSparseSamplePoints')));
        end
    end
    if any(diff(t) <= 0)
        if any(diff(t) == 0)
            error(message(string('MATLAB:smoothdata:SamplePointsDuplicate'), term));
        else
            error(message(string('MATLAB:smoothdata:SamplePointsSorted'), term));
        end
    end
end
...
% Check the tuning factor input
    ...
    if isfloat(smFactor)
        if ~isreal(smFactor)
            error(message(string('MATLAB:smoothdata:noComplex'), '''SmoothingFactor'''));
        end
    ...
    if ~isscalar(smFactor) || ~((isnumeric(smFactor) && ...
        ...
        error(message(string('MATLAB:smoothdata:invalidSmoothingFactor')));
    end
...
% Check the Savitzky-Golay degree
    ...
    if isfloat(sgdeg)
        if ~isreal(sgdeg)
            error(message(string('MATLAB:smoothdata:noComplex'), '''Degree'''));
        end
        if ~isfinite(sgdeg)
            error(message(string('MATLAB:smoothdata:needsFinites'), '''Degree'''));
        end
    end
    if ~isscalar(sgdeg) || ~isnumeric(sgdeg) || ...
        (fix(sgdeg) ~= sgdeg) || (sgdeg < 0)
        error(message(string('MATLAB:smoothdata:negativeDegree')));
    end
    ...
    if ~isempty(t)
        ...
        if (sgdeg >= wl)
            if autoSize
                ...
                if gaveDegree
                    error(message(string('MATLAB:smoothdata:degreeTooLarge'), wl));
                else
                    sgdeg = wl-1;
                end
            end
        ...
        if (sgdeg >= winsz)
            if autoSize
                ...
                sgdeg = fix(winsz-1);
                if gaveDegree
                    warning(message(string('MATLAB:smoothdata:degreeAutoClash'),...
                       winsz, sgdeg));
                end
            else
                if gaveDegree
                    error(message(string('MATLAB:smoothdata:degreeTooLarge'),...
                        winsz));
                else
                    sgdeg = fix(winsz-1);
                end
